...
def select_multiple_devices(devices):
    ...
    device_ids_str = input("For example: 0 1 2 (Enter the IDs): ")
...
def execute_device(device):
    ...
    while True:
        print("Waiting for Ranges from server...")
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                time.sleep(5)  # Esperar 5 segundos antes de volver a intentar obtener rangos del servidor
                continue  # Volver al inicio del bucle y esperar nuevamente
    ...
    try:
        ...
        if os.path.exists("selected_devices.txt"):
            with open("selected_devices.txt", "r") as f:
                devices_selected = [device.strip() for device in f.readlines()]
        else:
            devices_selected = select_multiple_devices(devices)
        ...
        threads = []
        for device in devices_selected:
            thread = threading.Thread(target=execute_device, args=(device,))
            thread.start()
